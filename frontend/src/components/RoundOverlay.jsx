// RoundOverlay.jsx
export function RoundOverlay({ p1Name, p2Name, p1Score, p2Score, p1RoundWins, p2RoundWins, roundActive, matchOver }) {
  // Show briefly when round just ended (roundActive becomes false) but match isn't over
  const show = !roundActive && !matchOver
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        color: isTie ? 'var(--neon-yellow)' : isP1Win ? 'var(--neon-cyan)' : 'var(--neon-red)',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, maxWidth: 360, width: '100%' }}>
        {[
          { name: p1Name, score: p1Score, color: 'var(--neon-cyan)' },
          { name: p2Name, score: p2Score, color: 'var(--neon-red)' }
...
export function MatchOverlay({ matchOver, winner, myName, p1Name, p2Name, p1Wins, p2Wins, onRematch, onLeave }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        fontFamily: "'Share Tech Mono',monospace",
        fontSize: 11, color: 'var(--dim)', letterSpacing: 4
      }}>
        MATCH CHAMPION
      </div>
      ...
      <div style={{
        fontFamily: "'Share Tech Mono',monospace",
        fontSize: 14,
        color: isMyWin ? 'var(--neon-green)' : isTie ? 'var(--neon-yellow)' : 'var(--neon-red)',
        letterSpacing: 2
      }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, maxWidth: 360, width: '100%' }}>
        {[
          { name: p1Name, wins: p1Wins, color: 'var(--neon-cyan)' },
          { name: p2Name, wins: p2Wins, color: 'var(--neon-red)' }
        ].map(({ name, wins, color }) => (
          <div key={name} style={{
            padding: 14, textAlign: 'center',
            border: `1px solid ${color}`,
            ...
            <div style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: 9, color: 'var(--dim)', letterSpacing: 2, marginTop: 4 }}>
              {name}<br />ROUNDS
            </div>
          </div>
        ))}
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 360, width: '100%' }}>
        <button onClick={onRematch} style={{
          flex: 1, minWidth: 130, padding: '13px 20px',
          background: 'var(--neon-cyan)', color: '#000',
          border: 'none', fontFamily: "'Black Ops One',cursive",
        ...
        <button onClick={onLeave} style={{
          ...
          border: '1px solid var(--neon-red)', fontFamily: "'Black Ops One',cursive",
          fontSize: 13, letterSpacing: 2, cursor: 'pointer',
          clipPath: 'polygon(0 0, calc(100% - 8px) 0, 100% 8px, 100% 100%, 8px 100%, 0 calc(100% - 8px))'
        }}>LEAVE</button>
      </div>
