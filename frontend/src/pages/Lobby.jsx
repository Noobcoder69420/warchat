...
export default function Lobby() {
  ...
  const [tab, setTab] = useState('create') // create | join | matchmaking
  ...
  function createRoom() {
    if (!hostName.trim()) {
      dispatch({ type: 'SET_STATUS', msg: 'ENTER YOUR FIGHTER NAME', stype: 'err' })
      return
    }
    socket.emit('create_room', { name: hostName.trim() })
  }
  ...
  function joinRoom() {
    ...
    if (!joinCode.trim() || joinCode.trim().length < 4) {
      dispatch({ type: 'SET_STATUS', msg: 'ENTER A VALID 4-CHARACTER ROOM CODE', stype: 'err' })
      return
    }
    socket.emit('join_room_req', { name: joinName.trim(), room_id: joinCode.trim().toUpperCase() })
  }
  ...
  function joinMatchmaking() {
    ...
    socket.emit('join_matchmaking', { name: mmName.trim() })
  }
  ...
  return (
    <div className={styles.page}>
      <div className={styles.container}>
        ...
        {/* Status */}
        {state.statusMsg && (
          <div className={`${styles.status} ${styles[state.statusType]}`}>
            {state.statusMsg}
          </div>
        )}
        ...
        <div className={styles.tabs}>
          {['create', 'join', 'matchmaking'].map(t => (
            <button
              ...
              onClick={() => setTab(t)}
            >
              {t === 'create' ? '⚡ CREATE' : t === 'join' ? '🔥 JOIN' : '🎯 MATCHMAKING'}
            </button>
          ))}
        </div>
            ...
            {!isWaiting ? (
              ...
              <div className={styles.waitingPanel}>
                ...
                <div className={styles.codeBox}>
                  <span className={styles.codeVal}>{state.roomId}</span>
                </div>
                ...
                <p className={styles.hint}>Share the code with your friend so they can join</p>
              </div>
            )}
          </div>
        )}
            ...
            {!state.matchmaking ? (
              <>
                ...
                <input
                  ...
                  onKeyDown={e => e.key === 'Enter' && joinMatchmaking()}
                  placeholder="e.g. TrashTalkGod"
                  maxLength={20}
                  disabled={!state.connected}
                />
                <button
                ...
                <p className={styles.hint}>Matched with a random player from the queue</p>
              </>
            ) : (
              <div className={styles.waitingPanel}>
                ...
                <button className={`${styles.btn} ${styles.btnGhost}`} onClick={cancelMatchmaking}>
                  CANCEL
                </button>
              </div>
            )}
        ...
        <p className={styles.footer}>
          POWERED BY FLASK · SOCKET.IO · GROQ AI
        </p>
      </div>
    </div>
  )
